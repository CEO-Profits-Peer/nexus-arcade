...
(function(){
  ...
  const SYNC_KEYS = ["nexus_profile","nexus_ach","nexus_quests","nexus_favs","nd_best","nd_lang","nd_upgrades","nr_save_v1","nr_lang","nw_lang","nw_v1_en","nw_v1_de","nw_v1_es","nx_racer_best","nx_racer_upg","nx_run3d_upg","nx_2048_best","nx_run3d_best","nx_snake_best","nx_breaker_best","nx_tycoon","nx_tycoon_best","nx_stack_best","nx_blocks_best","nx_finance_best","nx_finance_empire","nx_ticker_best","nx_lang","nx_muted","nexus_pro"];
  ...
  const UPGRADE_SYNC_KEYS = ["nd_upgrades","nx_racer_upg","nx_run3d_upg"];
  ...
  function renderModal(){
    ...
    if(activeTab==="profile"){
      body = '<div style="display:flex;align-items:center;gap:14px;margin-bottom:14px">'+
        ...
        '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px">'+
          ...
          (PRO_FRAME?'<button class="nxFrame" data-f="'+PRO_FRAME.id+'" data-pro="1" title="'+(isPro()?PRO_FRAME.label[L]:t.proCta)+'" style="width:40px;height:40px;border-radius:50%;background:#1a2140;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:11px;color:#fff;opacity:'+(isPro()?1:.4)+';border:3px solid #5a6187;'+PRO_FRAME.css+(profile.frame===PRO_FRAME.id?';outline:2px solid var(--gold,#ffcf5c);outline-offset:2px':'')+'">'+(isPro()?'✨':'🔒')+'</button>':'')+
        '</div>'+
    ...
    if(activeTab==="profile"){
      ...
      dom.card.querySelectorAll(".nxFrame").forEach(b=>{
        if(b.getAttribute("data-pro")==="1" && !isPro()){ b.onclick=()=>{ close(); location.href="/pro/"; }; return; }
        if(!b.disabled) b.onclick=()=>{profile.frame=b.getAttribute("data-f");saveProfile();syncPush();renderModal();renderButton();};
      });
  ...
  // Fuer coins-basierte Spiel-Shops (z.B. Dash-/Racer-Upgrades): zentral abziehen statt jedes
  // Spiel direkt localStorage["nexus_profile"] basteln zu lassen, damit Speichern+Sync (syncPush)
  // + der Konto-Button (renderButton) garantiert konsistent bleiben.
  function spendCoins(n){
    n=Math.max(0,Math.floor(n||0));
    if((profile.coins||0)<n) return false;
    profile.coins-=n; saveProfile(); syncPush(); renderButton();
    return true;
  }
